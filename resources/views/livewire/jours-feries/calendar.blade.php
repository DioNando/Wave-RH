<div>
    <div class="flex justify-between items-center mb-5">
        <div class="flex items-center">
            <div class="flex items-center gap-3">
                <x-button.action wire:click="prevYear" type="button" color="gray" icon="chevron-left" />
                ...
                <x-button.action wire:click="nextYear" type="button" color="gray" icon="chevron-right" />
    ...
    <x-card class="mx-auto p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @foreach ($months as $month)
            <div class="h-full">
                <div class="p-4 h-full flex flex-col">
                    <h2 class="font-semibold text-gray-900 dark:text-white text-center mb-3 uppercase text-sm">
                        {{ $month['name'] }}</h2>
...
$timeClasses = [
    ...
    $isToday ? 'text-green-600 font-semibold rounded-full' : '',
                            ...
                            <button type="button" class="{{ implode(' ', $buttonClasses) }}"
                                @if ($hasHoliday) x-data="{}"
                        x-tooltip.raw="{{ implode(', ',array_map(function ($holiday) {return $holiday->nom;}, $this->getJoursFeriesForDate($dayData['date']))) }}" @endif>
