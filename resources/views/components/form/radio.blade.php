@props(['name', 'value', 'checked' => false, 'label' => '', 'direction' => null])

<div class="flex items-center text-sm text-gray-900 dark:text-gray-100 gap-x-3">
    @if ($direction === 'asc')
        <input wire:model.live.250ms="{{ $name }}" id="{{ $name }}_{{ $value }}" name="{{ $name }}" value="{{ $value }}"
